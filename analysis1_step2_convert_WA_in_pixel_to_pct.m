...
Constant_rel_dis_btw_WA_LB = 0.385; % a number between 0 and 1, when taken 0 LB = WA, when taken 1 RB = WA 
...
for i = 1:t_videoinfo_size(1)
    ...
    RedWA = t_data.RedWA;
    RedWAPct =  (RedWA-l_boundary)/(r_boundary-l_boundary)*100;
    t_data.RedWAPct = RedWAPct;
    GreenWA = t_data.GreenWA;
    x = Constant_rel_dis_btw_WA_LB;
    GreenLBPct = ((GreenWA - x*r_boundary)/(1-x)-l_boundary)/(r_boundary-l_boundary)*100;
