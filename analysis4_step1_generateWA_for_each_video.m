...
inputFolder = '/path_to_folder/N2p/WT-1'; 
outputFolder ='/path_to_folder/N2p/WT-1'; 
inputFile = 'WT-1.avi';
...
for x = 1 : frame_num
    %Create a filename
    outputBaseFileName = sprintf('WT-1-%4.4d.tif', x);
...
for i = 1:frame_num
    t = Tiff(fullfile(outputFolder,['WT-1-',sprintf('%4.4d',i),'.tif']),'r');
    imageData = read(t);


    x_coordination = 1:350;
    ...
    hold on;
    box on;
    plot(1:350,Red_sum,'r-')
    %plot(1:350,Blue_sum*mean(Red_sum(400:500)./Blue_sum(400:500)),'r--')
    line([1 350],[Red_bkgl Red_bkgl],'Color','red','LineStyle','--')
    scatter(Red_WA(i),mean(Red_sum),20,'MarkerFaceColor',[1,0,0])
    
    plot(1:350,Green_sum,'g-')
    scatter(Green_WA(i),mean(Green_sum),20,'MarkerFaceColor',[0,1,0])
    line([1 350],[Green_bkgl Green_bkgl],'Color','green','LineStyle','--')

    %scatter(Blue_WA,mean(Blue_sum),20,'MarkerFaceColor',[0,0,1])
    %plot(1:350,Blue_sum,'b-')    
